...
def build_source_url(row: dict[str, Any], site_root: str) -> str:
    ...
    return f"{site_root.rstrip('/')}/api/profile/{int(row['runner_id'])}"
...
def load_year_races_from_dump(sql_path: Path, year: int) -> dict[int, dict[str, Any]]:
...
def load_rows_from_dump(sql_path: Path, year: int, synthetic_id_offset: int) -> list[dict[str, Any]]:
    selected_races = load_year_races_from_dump(sql_path, year)
...
def load_rows(source_dsn: str, year: int, synthetic_id_offset: int) -> list[dict[str, Any]]:
    import psycopg
    ...
    for athlete_id in sorted(grouped_rows):
        ...
        for row_order, row in enumerate(athlete_rows):
            ...
            result = {
                ...
                "venue_url": f"{site_root.rstrip('/')}/api/results/{int(row['race_id'])}",
...
def generate_sql(athletes: Sequence[AthleteExport], *, year: int, site_root: str) -> str:
...
JOIN athlete_performance_sections s
  ON s.athlete_id = r.athlete_id
 AND s.source_kind = 'truepb_results'
 AND s.year = {year}
 AND s.title = {sql_literal(title)};
...
def main() -> None:
    args = parse_args()
    if args.sql:
        rows = load_rows_from_dump(sql_path_from_args(args), args.year, args.synthetic_id_offset)
    else:
        rows = load_rows(source_dsn_from_args(args), args.year, args.synthetic_id_offset)
    ...
    sql_text = generate_sql(athletes, year=args.year, site_root=args.site_root)
